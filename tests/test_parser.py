...
def test_parse_create_tapered_bspline():
    ...
    with _patch(resp):
        result = parse_command("테이퍼드 컵 만들어줘", {})
...
def test_parse_create_cylinder_ring():
    ...
    with _patch(resp):
        result = parse_command("원통형 컵 링 손잡이로 만들어줘", {})
    assert result["intent"] == "create"
...
def test_parse_op_set_absolute():
    ...
    with _patch(resp):
        result = parse_command("원형 손잡이 지름을 10mm로 수정해줘", {"ring_outer_r": 19.5})
...
def test_parse_op_delta_positive():
    ...
    with _patch(resp):
        result = parse_command("원형 손잡이 지름을 15mm 더 크게 해줘", {"ring_outer_r": 19.5})
    change = result["changes"][0]
...
def test_parse_op_delta_negative():
    ...
    with _patch(resp):
        result = parse_command("높이를 5mm 줄여줘", {"height": 70.0})
    change = result["changes"][0]
    assert change["op"] == "delta"
...
def test_parse_op_missing_passes_through():
    ...
    resp = '{"intent": "modify", "body_template": null, "handle_template": null, "changes": [{"param": "height", "value": 90}]}'
    with _patch(resp):
        result = parse_command("높이 90", {"height": 70.0})
    change = result["changes"][0]
    # op 없이 통과 — 라우트가 "set"으로 기본 처리
